/*
** convert_base_x.c for convert_base_x in /home/bongol_b/rendu/Piscine_C_J08/ex_05
**
** Made by berdrigue bongolo-beto
** Login   <bongol_b@epitech.net>
**
** Started on  Thu Oct  8 18:40:38 2015 berdrigue bongolo-beto
** Last update Thu Jan  7 23:48:00 2016 Berdrigue BONGOLO BETO
*/
...
char	*convert_base_x(char *nbr, char *base_from, char *base_to)
{
  ...
  if (nbr[0] == '-' && set_sign(convert_res, &neg, &i, 0) == 1)
    nbr = &nbr[1];
  nbr_base = my_getnbr_base_x(nbr, base_from);
  if (nbr_base == 0)
    return (return_zerobase(base_to));
  baseto_len = my_strlen_x(base_to);
  while (nbr_base > 0)
    {
  ...
  convert_res[i] = 0;
  return (my_revstr_x(convert_res));
}
